...
def get_features(imgs, batch_size=100):
    resnet = models.resnet34(pretrained=True).eval().to(device)
    resnet_feature = nn.Sequential(resnet.conv1, resnet.bn1, resnet.relu,
                                   resnet.maxpool, resnet.layer1,
                                   resnet.layer2, resnet.layer3, resnet.layer4).eval().to(device)
...
def get_dataset(dataset_name):
    ...
    if dataset_name == 'lsun':
        lsun = dset.LSUN(root=g.default_data_dir+'lsun/', classes=['bedroom_train'],
                            transform=transforms.Compose([
                                transforms.Resize(64),
    ...
    if dataset_name == 'cifar10':
        # folder dataset
        cifar10 = dset.CIFAR10(root=g.default_data_dir, download=True,
                               transform=transforms.Compose([
                                transforms.Resize(64),
                                transforms.CenterCrop(64),
                                transforms.ToTensor(),
