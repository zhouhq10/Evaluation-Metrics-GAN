...
def DCGAN_cluster_main(opt):
    ...
    if opt.to(device):
        torch.to(device).manual_seed_all(opt.manualSeed)

    cudnn.benchmark = True

    if torch.to(device).is_available() and not opt.to(device):
    ...
    for epoch in range(opt.niter):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            clus_var = Variable(clus_batch.to(device), requires_grad=False)
            oh = torch.FloatTensor(batch_size, N_CLUSTER)
            oh.zero_()
            oh.scatter_(1, clus_batch.view(-1, 1), 1)
            oh_var = Variable(oh.to(device), requires_grad=False)
