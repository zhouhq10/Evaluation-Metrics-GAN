...
from utils import mkdir, TMix, TGen, TExp, TGan
...
if False:
    ...
    #tasks.append(TGen('Gen', 'celeba', 'DCGAN', 49, 'vgg16'))
    #tasks.append(TGen('Gen', 'cifar10', 'true', 49, 'vgg16'))
    #tasks.append(TGen('Gen', 'cifar10', 'DCGAN', 49, 'vgg16'))
    ...
    #tasks.append(TGen('Gen', 'celeba', 'DCGAN', 49, 'inception_v3'))
    #tasks.append(TGen('Gen', 'cifar10', 'true', 49, 'inception_v3'))
    #tasks.append(TGen('Gen', 'cifar10', 'DCGAN', 49, 'inception_v3'))


trans = transforms.Compose([transforms.Resize(64), transforms.ToTensor(
), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
trans_rand = transforms.Compose([transforms.Resize(64), transforms.Pad(4), transforms.RandomCrop(
    64), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
trans_rotate = transforms.Compose([transforms.Resize(
...
if False:
    for sampleSize in [2000]:
        for data in ['celeba']:#, 'celeba', 'lsun']:
            if data == 'celeba':
                model = 'DCGAN49'
            #if data == 'lsun':
            #    model = 'DCGAN9'
    ...
    for sampleSize in [100, 200, 400, 800, 1600, 3200, 6400, 12800]:
        for data in ['celeba']: #'celeba', 'lsun']:
            for featureType in ['resnet34']: #, 'vgg16', 'inception_v3']:
                for foldername in ['DCGAN49']: #if data == 'celeba' else 'DCGAN9', 'true_test']:
                    dataList = []
...
if False:
    for data in ['celeba']: #'celeba', 'lsun']:
        for exp in ['overfit']: #, 'drop', 'overfit']:
